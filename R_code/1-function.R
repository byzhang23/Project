...
income_md_dc <- function(x){
    sub_inc=x %>% filter(state %in% c("11","24"))
    return(sub_inc)
}

plots <- function(name,acc_file,year){
    #data
    usa = map_data("state")
    ...
    state_fata_rate<- state_base + 
        ...
        scale_fill_gradient(low = "yellow", high = "red",breaks=seq(0,0.4,by=0.05))+
    ...
    multiplot(state_fata_rate,state_inc,cols = 1)
}
